...
function drawOrders(data) {
    let place = document.getElementById("lastOrder");
    if (data.length !== 0) {
        let elem = "";
        elem = '<div class="row" style="background: #1959d1"><h1 class="centerh" style="color: white">Ваши заказы:</h1></div>';
        elem += '<table class="centerh" style="text-align: center;">';
        for (let i = 0; i < data.length; i++) {


            elem += '<tr><td>' + data[i]['dish']['group']['name'] + '</td>' +
                '<td><h1>' + data[i]['dish']['name'] + ' </h1></td>' + '<td></td>' +
                '<td>' + data[i]['dish']['price'] + '₽' + '</td>' +
                '<td> </td>' + '<td>Количество:</td>' +
                '<td>' + data[i]['count'] + '</td>';
        ...
        elem += '</table>';
        place.innerHTML = elem;

    } else {
        let elem = '<h1 class="centerh" style="margin-left: 20%">У вас еще не было заказов</h1>'
        place.innerHTML = elem;
